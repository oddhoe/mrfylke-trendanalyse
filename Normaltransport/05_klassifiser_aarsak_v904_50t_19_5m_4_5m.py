...
Klassifiserer flaskehalser fra 04 med årsak basert på dimensjonerende verdi:
  ...
  - Hvis bru < 60t men >= 50t og ikke allerede "BRU" => "BRU60"
  - Evt. "LENGDE" og/eller "HØYDE" hvis under terskel.

Slår opp mot Veg_TillatProfil for råverdier.
"""
...
# --- KRAV ---
VEKT_KRAV     = 50.0
BRU_TONN_KRAV = 60.0   # ny: bru under 60t er selvstendig begrensningstype
LENGDE_KRAV   = 19.5
HOYDE_KRAV    = 4.5
...
need = [
    ("AARSAK_DETALJERT",  "TEXT",   100),   # økt fra 80 for BRU60-kombinasjoner
...
if not any([P_BK, P_BRU, P_LEN, P_HOY]):
    raise RuntimeError(
        "Fant ingen relevante profil-felt (BK_VERDI / MIN_BRU_TONN / MAKS_LENGDE / MIN_HOYDE)."
    )

# ------------------------------
# BYGG OPPSLAG FRA PROFIL
# ------------------------------
print("Bygger oppslag (per veglenke) fra Veg_TillatProfil...")
idx  = {}
...
with arcpy.da.UpdateCursor(OUT_FC, out_fields) as ucur:
    for vls, s0, s1, aarsak, veg_bk, bru_tonn, maks_len, fri_hoyde in ucur:
        ...
        # --- Bru < 60t: selvstendig begrensning (bru 50–59t) ---
...
print(f"✅ Ferdig! Oppdaterte {updated} rader, {no_hit} uten profil-treff.")
